...
// what you fought for before the gate comes back.
...
// The per-navigation dwell pass-through (see longFormDwellMin) judges each
// hop on its own: one long video followed by a short glance still gates the
// hop after that, even though the first video already showed real
// engagement. A qualifying long-form session banks the same grace/trust
// credit an override does — smaller, since it's a passive signal rather than
// a deliberate costly action, but it means one genuinely engaging video buys
// some slack for what you click next, not just a single hop's exemption.
export const DEFAULT_LONG_FORM_GRACE_MIN = 10;
export const DEFAULT_LONG_FORM_TRUST_BOOST = 0.35;
...
export function defaultSettings() {
  return {
    ...
    longFormGraceMin: DEFAULT_LONG_FORM_GRACE_MIN,
    longFormTrustBoost: DEFAULT_LONG_FORM_TRUST_BOOST,
  };
}
...
export function computeGrantReward(activeMinutes, recentSessionCount, wasOverride, settings) {
  ...
  const reward = -durationPenalty - frequencyPenalty - overridePenalty;
  return Math.max(-1, Math.min(0, reward));
}
