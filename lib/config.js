...
// *Committing* to a break isn't a bandit decision (see startBreak's
// finalizing of any in-progress grant, which trains the site bandit
// normally on real usage, but never touches break state) — but *how long*
// the break should be is, exactly the way a site's grant duration is: see
// the break-duration bandit constants further down.
//
...
// A separate LinUCB instance (see getBreakBandit in lib/background-helpers.js)
// picks the suggested break length from this fixed candidate list, the same
// way the site bandit picks a grant duration from armDurationsMin — see
// docs/adr/0003-break-duration-bandit-and-fatigue-feature.md for the
// simulation this is based on. The candidate list itself stays fixed
// regardless of breakMaxMin (persisted bandit state needs a stable arm
// count); breakMaxMin instead filters which candidates are eligible to be
// suggested or selected — see eligibleBreakArms below.
export const BREAK_DURATIONS_MIN = [10, 20, 30, 45, 60, 90, 120];

// ADR 0003 found the site bandit's tuned alpha does not transfer here —
// different reward scale (up to +0.5, not +0.15), different arm count and
// spacing. 0.15 sits in the middle of the 0.08-0.3 range that simulation
// found roughly halved regret versus alpha=1.0 - deliberately not the
// single best value found (0.08), for the same "don't ship a synthetic
// optimum as a real default" reasoning as DEFAULT_ALPHA in ADR 0001.
export const DEFAULT_BREAK_ALPHA = 0.15;
export const DEFAULT_BREAK_DISCOUNT_FACTOR = 0.99;

// Reward for how a break actually held up, computed once its outcome is
// known (see computeBreakReward below and onBreakFollowup in background.js
// for when each branch fires):
// - Overridden before it ended: a penalty scaled by how early the override
//   happened — breaking at minute 2 of 60 is worse than breaking at minute
//   55, same asymmetry ADR 0003's simulation validated.
...
//   wasn't long enough — a penalty scaled by how soon you came back.
// - Not overridden, and no managed-site activity within that window: a
//   clean completion, the full bonus.
export const DEFAULT_BREAK_COMPLETE_BONUS = 0.5;
export const DEFAULT_BREAK_TOO_SHORT_PENALTY = 0.6;
export const DEFAULT_BREAK_OVERRIDE_PENALTY = 1.0;

// How soon after a break ends does re-engaging with a managed site count as
// "that break was too short," rather than just... the break being over and
// life continuing normally. Long enough to distinguish "immediately went
// right back" from an unrelated visit hours later.
export const DEFAULT_BREAK_TOO_SOON_WINDOW_MIN = 10;

// Total minutes across all managed sites in the last 24h at which the
// popup starts proactively suggesting a break, rather than only offering
// one when asked. suggestCooldown keeps that from nagging on every popup
// open while the total stays elevated.
export const DEFAULT_BREAK_EFFORT_THRESHOLD_MIN = 90;
export const DEFAULT_BREAK_SUGGEST_COOLDOWN_MIN = 60;
...
export function defaultSettings() {
  return {
    ...
    breakAlpha: DEFAULT_BREAK_ALPHA,
    breakDiscountFactor: DEFAULT_BREAK_DISCOUNT_FACTOR,
    breakCompleteBonus: DEFAULT_BREAK_COMPLETE_BONUS,
    breakTooShortPenalty: DEFAULT_BREAK_TOO_SHORT_PENALTY,
    breakOverridePenalty: DEFAULT_BREAK_OVERRIDE_PENALTY,
    breakTooSoonWindowMin: DEFAULT_BREAK_TOO_SOON_WINDOW_MIN,
    breakEffortThresholdMin: DEFAULT_BREAK_EFFORT_THRESHOLD_MIN,
    breakSuggestCooldownMin: DEFAULT_BREAK_SUGGEST_COOLDOWN_MIN,
  };
}

// eligibleBreakArms filters BREAK_DURATIONS_MIN's fixed indices down to the
// ones allowed by the current breakMaxMin — the bandit's arm count itself
// never changes (persisted state needs a stable shape), only which of its
// arms can actually be suggested or selected.
export function eligibleBreakArmIndices(settings) {
  return BREAK_DURATIONS_MIN.map((_, i) => i).filter((i) => BREAK_DURATIONS_MIN[i] <= settings.breakMaxMin);
}

// The real (non-simulated) counterpart to sample_break_reward_batch in
// eval/simulate_break.py — computed once from an observed outcome rather
// than sampled, since there's no latent "actually needed" duration to draw
// from in reality, only what was actually observed: whether it was
// overridden, and if not, whether there was a too-soon return. See the
// constants above for what each branch means.
export function computeBreakReward(outcome, params, settings) {
  if (outcome === 'overridden') {
    const elapsedFrac = Math.max(0, Math.min(1, params.elapsedFrac));
    return Math.max(-1, -settings.breakOverridePenalty * (1 - elapsedFrac));
  }
  if (outcome === 'too_short') {
    const shortfallFrac = Math.max(0, Math.min(1, params.shortfallFrac));
    return Math.max(-1, settings.breakCompleteBonus - settings.breakTooShortPenalty * shortfallFrac);
  }
  return settings.breakCompleteBonus; // 'completed' cleanly
}
