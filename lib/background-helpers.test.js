...
import { BREAK_DURATIONS_MIN } from './config.js';
...
test('getBreakBandit constructs a fresh bandit sized to BREAK_DURATIONS_MIN with no saved state', () => {
  const store = { breakBanditState: null, settings: { breakAlpha: 0.15, breakDiscountFactor: 0.99 } };
  const bandit = getBreakBandit(store);
  assert.equal(bandit.arms.length, BREAK_DURATIONS_MIN.length);
  assert.equal(bandit.alpha, 0.15);
});

test('getBreakBandit restores saved state and reapplies current settings', () => {
  const fresh = getBreakBandit({ breakBanditState: null, settings: { breakAlpha: 0.15, breakDiscountFactor: 0.99 } });
  fresh.update(0, [1, 0, 0, 0, 0, 0, 0], 0.3);
  const saved = fresh.toJSON();

  const restored = getBreakBandit({ breakBanditState: saved, settings: { breakAlpha: 0.4, breakDiscountFactor: 0.9 } });
...
test('getBreakBandit starts fresh if the saved arm count no longer matches BREAK_DURATIONS_MIN', () => {
  const stale = { d: 7, alpha: 1, gamma: 1, arms: [{ A: [[1]], b: [0] }] }; // wrong arm count
  const bandit = getBreakBandit({ breakBanditState: stale, settings: { breakAlpha: 0.15, breakDiscountFactor: 0.99 } });
  assert.equal(bandit.arms.length, BREAK_DURATIONS_MIN.length);
});
