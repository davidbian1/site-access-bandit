...
import { minutesToMs, FREE_TIME_DURATIONS_MIN } from './config.js';
...
// suggested free-time duration — see FREE_TIME_DURATIONS_MIN's comment in
// lib/config.js and docs/adr/0003-break-duration-bandit-and-fatigue-feature.md's
// Update section. Its arm count is always FREE_TIME_DURATIONS_MIN.length
// regardless of the current freeTimeMaxMin setting; eligibleFreeTimeArmIndices
// (lib/config.js) filters which of those arms are actually selectable at
// suggestion time.
export function getFreeTimeBandit(store) {
  const { freeTimeBanditState, settings } = store;
  const nArms = FREE_TIME_DURATIONS_MIN.length;
  if (freeTimeBanditState && freeTimeBanditState.arms.length === nArms && freeTimeBanditState.d === FEATURE_DIM) {
    return LinUCB.fromJSON({ ...freeTimeBanditState, alpha: settings.freeTimeAlpha, gamma: settings.freeTimeDiscountFactor });
  }
  return new LinUCB(nArms, FEATURE_DIM, settings.freeTimeAlpha, settings.freeTimeDiscountFactor);
}
...
export function makeGrant(armIndex, durationMin, context, now, targetUrl, extra = {}) {
  return {
    ...
    activeSeconds: 0,
    targetUrl,
    // A grant is requested from wherever you happened to be blocked - often
    // a general entry point (a site's homepage, a search/listing page), not
    // the specific piece of content you actually meant to reach. Locking
    // isSameSubUrl enforcement to that literal starting URL would gate the
    // very first click into real content, immediately after being granted -
    // exactly the interruption this whole mechanism exists to avoid. One
    // hop is spent for free on the first real navigation after a grant is
    // made (see CHECK_ACCESS in background.js), which is also where
    // targetUrl gets updated to whatever page that navigation actually
    // lands on - from then on, that's the one sub-URL this grant covers.
    hopUsed: false,
    ...extra,
  };
}
...
// How many times gating actually pushed back today, across every managed
// site — a denial that stood, or one that only gave way to an override.
// This is what drives the free-time proactive-suggestion trigger (see
// GET_FREE_TIME_SUGGESTION in background.js): total time spent (above)
// is a reasonable predictor of how *long* a free-time window should be,
// but it's not the same thing as how often the gate has actually been
// fought today, which is the more direct signal for *whether* to offer
// one right now — see DEFAULT_FREE_TIME_FRICTION_THRESHOLD's comment in
// lib/config.js.
export function globalFrictionCount24h(sessions, nowMs) {
  const dayAgo = nowMs - 24 * 60 * 60 * 1000;
  let count = 0;
  for (const s of sessions) {
    if (s.grantedAt >= dayAgo && (s.decision === 'deny' || s.overridden)) count += 1;
  }
  return count;
}
