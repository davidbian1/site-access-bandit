...
class LinUCBArm {
  ...
  update(x, reward) {
    this.A = matAdd(this.A, outer(x, x));
    this.b = vecAdd(this.b, vecScale(x, reward));
  }
}

export class LinUCB {
  constructor(nArms, d = FEATURE_DIM, alpha = 1.0) {
    this.d = d;
    this.alpha = alpha;
  ...
  static fromJSON(o) {
    const bandit = new LinUCB(o.arms.length, o.d, o.alpha);
    bandit.arms = o.arms.map((a) => LinUCBArm.fromJSON(a));
    return bandit;
  }

  toJSON() {
    return { d: this.d, alpha: this.alpha, arms: this.arms.map((a) => a.toJSON()) };
  }
  ...
  update(armIndex, x, reward) {
    this.arms[armIndex].update(x, reward);
  }
}
